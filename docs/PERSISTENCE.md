...
10. Restore the backup if step 9 fails; otherwise remove it.
...
11. Validate supersession relationships, status, ancestry, TRACE, and acyclicity.
...
15. Validate session/project identity, draft schema, and source metadata.
...
Round lock clones the active workspace through the same plain-data codecs, applies the complete draft to that clone, and asks the store to validate and promote it. If synthesis, supersession, concept lifecycle, encoding, hashing, validation, or promotion fails, the active workspace reference is never replaced. The already-autosaved draft remains available for correction.
